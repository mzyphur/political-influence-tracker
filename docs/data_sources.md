...
| `abs_data_api` | ABS Data API (SDMX, beta) | https://data.api.abs.gov.au/rest/ | Detailed economic, social, and Census data | Beta service; availability not guaranteed; subject to change. Keyless since 29 Nov 2024 (register-of-interest model). Distinct from the ABS Indicator API: do not send `ABS_API_KEY` here. SDMX-compliant; XML, JSON, or CSV via `format=` or `Accept` header. Reuse/licence terms must be re-confirmed against current ABS terms before public data redistribution. |
