...
| `abs_indicator_api` | ABS Indicator API | https://api.data.abs.gov.au/indicators | Headline market-moving statistics | Production-suitable. Requires `ABS_API_KEY`. Rate limit 10 calls/sec/key. Headline indicators publish at 11:30am AEST on release day. Reuse/licence terms must be re-confirmed against current ABS terms before public data redistribution. |
| `abs_data_api` | ABS Data API (SDMX, beta) | https://data.api.abs.gov.au/rest/ | Detailed economic, social, and Census data | Beta service; availability not guaranteed; subject to change. Keyless since 29 Nov 2024 (register-of-interest model). Distinct from the ABS Indicator API: do not send `ABS_API_KEY` here. SDMX-compliant; XML, JSON, or CSV via `format=` or `Accept` header. Public redistribution/licence terms to be recorded before public data redistribution. |
...
## Postcode Seed (Comprehensive)

The default postcode seed (`data/seeds/aec_postcode_search_seed.txt`)
is intentionally short — 48 capital-CBD + regional-centre postcodes
covering ~80% of the Australian population — so routine pipeline runs
respect AEC endpoint etiquette.

The full ~9000-postcode comprehensive seed is at
`data/seeds/aec_postcode_search_seed_full.txt`, regenerated by
`scripts/build_postcode_seed_from_cc0.sh`. The source is **Matthew
Proctor's `matthewproctor/australianpostcodes` GitHub dataset**, which
is published under the **Public Domain (CC0)** dedication. The
build script archives the source CSV under
`data/raw/cc0_postcode_seed_source/<timestamp>/` with a SHA-256
metadata file so the seed list itself is reproducible from a fixed
upstream snapshot.

**Why not Australia Post.** Australia Post's free postcode CSV is
licensed for non-commercial reference only, and explicitly blocks
sub-licensing or use to power any publicly available postcode lookup.
See `docs/source_licences.md` for the verbatim restriction.

**Operational note.** Stage bulk runs against the live AEC Electorate
Finder via:

```bash
make expand-postcode-seed \
    SEED_FILE=data/seeds/aec_postcode_search_seed_full.txt \
    EXPAND_POSTCODE_SEED_FLAGS=--max-postcodes=200
```

A full ~9000-postcode national fetch is intentionally a maintainer-
scheduled task, not part of the weekly federal pipeline; respect AEC
endpoint etiquette and stage in 100-300 postcode batches.
