CREATE TABLE IF NOT EXISTS postcode_electorate_crosswalk_unresolved (
    id BIGSERIAL PRIMARY KEY,
    postcode TEXT NOT NULL CHECK (postcode ~ '^[0-9]{4}$'),
    electorate_name TEXT NOT NULL,
    state_or_territory TEXT NOT NULL DEFAULT '',
    ...
    UNIQUE (postcode, electorate_name, state_or_territory, match_method)
);

CREATE INDEX IF NOT EXISTS postcode_electorate_crosswalk_unresolved_postcode_idx
    ON postcode_electorate_crosswalk_unresolved (postcode);

CREATE INDEX IF NOT EXISTS postcode_electorate_crosswalk_unresolved_source_document_idx
    ON postcode_electorate_crosswalk_unresolved (source_document_id);
